...
class MultiTenantIsolationTest(TestCase):
    def setUp(self):
        ...
        self.user_a = User.objects.create_user(
            username="admin_a", password="pass", role=User.Role.ADMIN, tenant=self.tenant_a
        )
        self.seller_a = Seller.objects.create(
            tenant=self.tenant_a, name="Vendedor A", phone="1111-1111", user=self.user_a
        )

        self.user_b = User.objects.create_user(
            username="admin_b", password="pass", role=User.Role.ADMIN, tenant=self.tenant_b
        )
        self.seller_b = Seller.objects.create(
            tenant=self.tenant_b, name="Vendedor B", phone="2222-2222", user=self.user_b
        )
...
class DataMigrationTest(TestCase):
    ...
    def test_generate_unique_usernames_no_collision(self):
        ...
        existing = set()
        self.assertEqual(generate_unique_username("Maria", existing), "maria")
        self.assertIn("maria", existing)

        self.assertEqual(generate_unique_username("Maria", existing), "maria-2")
        self.assertIn("maria-2", existing)

        self.assertEqual(generate_unique_username("Léo", existing), "leo")
        self.assertIn("leo", existing)

        self.assertEqual(generate_unique_username("Leonardo Bruno", existing), "leonardo-bruno")
        self.assertIn("leonardo-bruno", existing)

    def test_link_sellers_to_users_creates_unique_usernames(self):
        """Integration test: verifies migration function creates Users with unique usernames."""
        link_sellers_to_users = self.migration.link_sellers_to_users

        tenant = Tenant.objects.create(company_name="Bibelo", cnpj="33333333333333")
        user_maria = User.objects.create_user(
            username="_temp_maria", password="pass", role=User.Role.SELLER, tenant=tenant
        )
        user_jose = User.objects.create_user(
            username="_temp_jose", password="pass", role=User.Role.SELLER, tenant=tenant
        )
...
class SellerCommissionRecalculateTest(TestCase):
    def setUp(self):
        self.tenant = Tenant.objects.create(company_name="Bibelo", cnpj="44444444444444")
        self.user = User.objects.create_user(
            username="vendedor_teste", password="pass", role=User.Role.SELLER, tenant=self.tenant
        )
        self.seller = Seller.objects.create(
            tenant=self.tenant, name="Vendedor Teste", phone="3333", user=self.user,
    ...
    def test_recalculate_sums_sales_correctly(self):
        Sale.objects.create(
            tenant=self.tenant,
            seller=self.seller,
            origin=Sale.Origin.MANUAL,
        ...
        self.assertEqual(self.sc.total_sold_amount, 30000)  # 100+200 (Julho excluido)
        self.assertEqual(self.sc.commission_amount, int(30000 * 0.05))  # 1500
