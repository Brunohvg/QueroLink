...
class PagarMeCustomerMixin:
    """Mixin for operations that use the Vidalys API key instead of tenant key."""

    def __init__(self, *args, **kwargs):
        api_key = kwargs.pop('api_key', None) or getattr(settings, 'VIDALYS_PAGARME_API_KEY', '')
        if not api_key:
            raise PagarMeError("Chave de API do Pagar.me nao configurada.")
        self.api_key = api_key


class PagarMeBillingGateway(PagarMeCustomerMixin):
    """Gateway for billing/subscription operations using Vidalys API key."""

    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        base = "https://api.pagar.me/core/v5"
        self.api_url_subscriptions = f"{base}/subscriptions"
        self.api_url_customers = f"{base}/customers"
        self.timeout = getattr(settings, 'PAGARME_TIMEOUT', 30)
    ...
    def _request(self, method, url, **kwargs):
        ...
        try:
            ...
            raise PagarMeError("Timeout ao comunicar com Pagar.me.")
        except requests.exceptions.RequestException as e:
            raise PagarMeError(f"Erro no gateway de billing: {e}")
        except ValueError:
            raise PagarMeError("Resposta invalida do gateway de billing.")
    ...
    def create_customer(self, name, email=None, phone=None):
        ...
        if email:
            payload['email'] = email
        if phone:
            payload['phones'] = [{'phone_type': 'mobile', 'ddd': phone[:2], 'number': phone[2:]}]
        return self._request("POST", self.api_url_customers, json=payload)

    def create_subscription(self, customer_id, plan_amount, billing_cycle, card_token=None, payment_method='boleto'):
        payload = {
            ...
            'items': [{
                'amount': plan_amount,
                'description': f'Plano {billing_cycle}',
                'quantity': 1,
                'pricing_scheme': {'scheme_type': 'UNIT', 'price': plan_amount},
                'name': 'Assinatura V-Com',
            }],
        }
        if card_token:
            payload['card_token'] = card_token
        return self._request("POST", self.api_url_subscriptions, json=payload)

    def cancel_subscription(self, subscription_id):
        return self._request(
            "DELETE", f"{self.api_url_subscriptions}/{subscription_id}",
        )

    def get_subscription(self, subscription_id):
        return self._request(
            "GET", f"{self.api_url_subscriptions}/{subscription_id}",
        )
